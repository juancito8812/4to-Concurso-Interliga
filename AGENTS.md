...
| `src/lib/espnResultsFetcher.ts` | Partidos finalizados ESPN en vivo para el cliente (caché 30s, `AbortSignal.timeout(10s)`, rango de fechas últimos 3 días) |
...
| `scripts/auto-sync-espn-results.js` | Cron: ESPN (backfill 3 días) → JSON evaluados → persistencia en Supabase con service role key. Usa rango `dates=YYYYMMDD-YYYYMMDD` (ESPN rechaza comas con HTTP 400) y **fail-fast**: si todas las ligas fallan, el run de Actions queda marcado como fallido |
...
| `scripts/sync-db.js` | Sincroniza Supabase: upsert de matches (sin pisar resultados), remapeo de predicciones a IDs reales, rebuild de la tabla teams (225 equipos) |
...
- La sección de premios en la landing (`src/app/page.tsx`) presenta un Podio de Campeones con medallas de texto (1° Oro, 2° Plata, 3° Bronce) y chips visuales independientes para cada artículo del kit. No se usan emojis informativos en la UI.
- **Política de emojis (Jul 2025):** No se usan emojis informativos (🏆👑🎯✅⚠️📊🔍 etc.) en la UI. Se permiten: emojis de acción (✕ ← →), emojis de premios (🎽 🩳 🧢 🚩 🍺 🕶️ 🖼️), e iconos decorativos en empty states dimmados (`text-silver/30`).
...
- **Resultados no sincronizados (incidente ago-2026):** ESPN cambió su API de scoreboard y **rechaza listas de fechas separadas por coma** (`?dates=20260831,20260830,...` → HTTP 400 `Failed to get events endpoint`); solo acepta fecha única o **rango con guión** (`?dates=YYYYMMDD-YYYYMMDD`). El cron y el fetcher del cliente usan rango con guión (backfill 3 días), y el cron tiene **fail-fast**: si todas las ligas devuelven error, el run de GitHub Actions queda marcado como fallido (antes fallaba en silencio y no sincronizaba nada). Para diagnosticar un run fallido: `gh run list --workflow=auto-evaluate-matches.yml` y revisar el log de "Sync finished match results from ESPN" (debe mostrar `HTTP 4xx` por liga con la URL usada). Verificar manualmente el formato con: `curl "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard?dates=20260828-20260831"`.
