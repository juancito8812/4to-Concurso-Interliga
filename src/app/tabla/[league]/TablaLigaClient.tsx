...
export default function TablaLigaClient() {
  ...
  useEffect(() => {
    ...
    const fetchStandings = async () => {
      try {
        const url = `https://site.api.espn.com/apis/v2/sports/soccer/${data.espnCode}/standings`;
        ...
        if (json.children && json.children[0] && json.children[0].standings) {
          ...
          console.log("Teams found:", entries.length);
          ...
          const mapped = entries.map((entry: any) => {
            ...
            return {
              rank: getStat("rank"),
              team: {
                name: entry.team?.displayName || entry.team?.shortDisplayName || "Unknown",
                    ...
                    <tr key={s.rank} className="border-b border-border/50 hover:bg-navy-card transition-colors">
                      ...
                      <td className="px-2 sm:px-4 py-2.5 sm:py-3">
                        <div className="flex items-center gap-2 sm:gap-3">
                          {s.team.logo && (
                            <img src={s.team.logo} alt="" className="w-5 h-5 sm:w-6 sm:h-6 object-contain" />
                          )}
