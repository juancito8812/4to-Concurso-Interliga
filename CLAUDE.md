...
- Cron GitHub Actions cada 2h (`auto-evaluate-matches.yml`) + Cron mensual de plantillas (`auto-sync-squads.yml`)
...
| `src/data/officialFixtures.json` | 1.672 partidos reales pre-sincronizados |
| `src/data/teamAliases.json` | 404 aliases, 241 equipos, 225 teamCups |
| `src/data/officialPlayers.json` | 7.097 jugadores clasificados por posición |
...
- Tablas: `teams` (225), `profiles`, `players`, `matches` (1.672), `predictions`, `prediction_scorers`, `tournament_survivors` (7 copas KO), `app_meta`.
